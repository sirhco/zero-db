...
pub fn build(b: *std.Build) void {
    ...
    const exe = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            ...
            // root module.
            .imports = &.{
                // Here "zero_db" is the name you will use in your source code to
                // import this module (e.g. `@import("zero_db")`). The name is
                // repeated because you are allowed to rename your imports, which
                // can be extremely useful in case of collisions (which can happen
                // importing modules from different packages).
                .{ .name = "zero_db", .module = mod },
            },
        }),
    });
    ...
    // Integration tests in tests/ — exercise only the public surface of the
    // `zero_db` module. Kept as a separate artifact so a failure here can't
    // be hidden by the inline unit tests inside src/.
    const integration_mod = b.createModule(.{
        .root_source_file = b.path("tests/sstable_index_test.zig"),
        .target = target,
        .imports = &.{
            .{ .name = "zero_db", .module = mod },
        },
    ...
    // times and since the run steps do not depend on one another, they run
    // in parallel.
